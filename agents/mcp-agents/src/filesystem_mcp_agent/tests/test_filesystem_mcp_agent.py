#!/usr/bin/env python3
"""
Test suite for Filesystem MCP Agent
...
class TestFilesystemMCPAgent:
    """Test class for Filesystem MCP Agent"""
    ...
    def test_yaml_syntax(self):
        ...
        try:
            ...
            # Check for filesystem_mcp_agent_app
            app_found = False
            for app in config['apps']:
                if app.get('name') == 'filesystem_mcp_agent_app':
                    app_found = True
                    break
            
            assert app_found, "filesystem_mcp_agent_app not found in configuration"
    ...
    def test_agent_configuration_structure(self):
        ...
        # Find the filesystem_mcp_agent_app
        app_config = None
        for app in config['apps']:
            if app.get('name') == 'filesystem_mcp_agent_app':
        ...
        assert app_config is not None, "filesystem_mcp_agent_app configuration not found"
        ...
        # Check agent name
        assert app_config['agent_name'] == 'FilesystemMCPAgent', f"Expected agent_name 'FilesystemMCPAgent', got '{app_config['agent_name']}'"
        
        # Check display name
        assert app_config['display_name'] == 'File System MCP Agent', f"Expected display_name 'File System MCP Agent', got '{app_config['display_name']}'"
    ...
    def test_agent_instructions(self):
        ...
        assert 'Filesystem MCP Agent' in instruction, "Instructions should mention 'Filesystem MCP Agent'"
        assert 'file system' in instruction.lower(), "Instructions should mention file system"
        assert 'directory' in instruction.lower(), "Instructions should mention directory operations"
        assert 'read' in instruction.lower(), "Instructions should mention read capabilities"
        assert 'write' in instruction.lower(), "Instructions should mention write capabilities"
    ...
    def test_agent_card_configuration(self):
        ...
        assert 'file system' in description.lower(), "Description should mention file system"
